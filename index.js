		...
		try {
			...
			mess = {
				...
				only: {
					...
					ownerG: 'grupo proprietário! ❌',
					ownerB: '[❗] Este comando só pode ser usado pelo Toin! ❌',
					admin: '[❗] Este comando só pode ser usado por administradores de grupo! ❌',
					Badmin: '[❗] Este comando só pode ser usado quando o bot se torna administrador! ❌',
                                        cadastroB: `── 「REGISTRE-SE」 ──\nSLV mano como!\nVocê nn está registrado no banco de dados Do Zuckzin Lisho tem que se cadastrar pra ter acesso ao menu, \n\nComando : ${prefix}cadastro nome|idade\nExemplo : ${prefix}cadastro zuckzin|14`,
				}
			}
			...
			const ownerNumber = ["556392942637@s.whatsapp.net"] // replace this with your number
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '556392942637@s.whatsapp.net'
                        /******ApiKey Input******/
  ...
  client.updatePresence(from, Presence.composing) 
	...
	case 'hidetag':
		...
		case 'spamcall':
               client.updatePresence(from, Presence.composing)
                 if (!isUser) return reply(mess.only.daftarB)
                 if (args.length < 1) return reply(`Insira numeros\nExemplo : ${prefix}spamcall 55639294...`)
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'block':
					...
					client.sendMessage(from, ` ,  ${body.slice(8)}@c.us`, text)
					break
				case 'hilih':
					client.updatePresence(from, Presence.composing) 
	          ...
	          if (!isUser) return reply(mess.only.daftarB)
               ...
               if (anu.error) return reply(anu.error)
				...
				case 'logowolf':
					...
					if (args.length < 1) return reply(`onde está o texto? exemplo ${prefix}logowolf ZUCKZIN|LINDO`)
					reply(mess.wait)
				...
				case 'logowolf2':
					...
					var gbl1 = gh.split("|")[0];
					var gbl2 = gh.split("|")[1];
					if (args.length < 1) return reply(`Cadê o texto, hum\nExemplo: ${prefix}phlogo |ZUCKZIN|LINDO`)
					reply(mess.wait)
					...
					case 'nsfwarmpits':
				        ...
				        uptime = process.uptime()
                                ...
                                case 'lionlogo':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        var teks2 = gh.split("|")[1];
                                        if (args.length < 1) return reply(`teksnya mana um\nContoh: ${prefix}lionlogo ZUCKZIN|LIXOS`)
                                        reply(mess.wait)
